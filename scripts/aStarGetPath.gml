...
startX=startRoomX div obPath.blockSize;
startY=startRoomY div obPath.blockSize;
endX=endRoomX div obPath.blockSize;
endY=endRoomY div obPath.blockSize;
...
if(found){
    ...
    while(curNode!=getKey(startX,startY)){
        path_add_point(path,getKeyX(curNode)*obPath.blockSize,
            getKeyY(curNode)*obPath.blockSize,100);
        curNode=ds_map_find_value(P,curNode);
    }
    path_add_point(path,startX*obPath.blockSize,startY*obPath.blockSize,100);
    path_reverse(path);
